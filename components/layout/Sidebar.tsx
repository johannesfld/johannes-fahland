    ...
    >
      <div className="flex h-full flex-col gap-4 overflow-y-auto px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pl-[max(0.75rem,env(safe-area-inset-left))] pt-5">
        ...
        <div className="flex items-center gap-2.5 px-1">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[var(--vibe-r-md)] bg-[var(--plum-50)] dark:bg-[var(--plum-950)] text-[var(--plum-500)]">
            <VibeLogo size={22} />
          </div>
        ...
        <nav className="flex flex-col gap-0.5" aria-label="Hauptnavigation">
          {NAV.map((item, index) => {
            ...
            return (
              ...
              >
                {active && (
                  <span className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-[3px] rounded-r-full bg-[var(--plum-500)]" />
                )}
                <item.icon
                  size={17}
                  className={cn(
                    "shrink-0 transition-colors",
                    active ? "text-[var(--plum-500)]" : "opacity-50",
                  )}
                  aria-hidden
                />
        ...
        {/* Footer */}
        <div className="flex items-center justify-between rounded-[var(--vibe-r-md)] border border-[var(--vibe-line)] bg-[var(--vibe-bg-sunken)]/50 px-3 py-2">
