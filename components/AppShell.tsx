...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-dvh min-h-0 w-full min-w-0 flex-row overflow-hidden bg-[var(--background)] text-[var(--foreground)]">
      {!fullscreen ? (
        <DesktopSidebar pathname={pathname} desktopSidebarOpen={desktopSidebarOpen} />
      ) : null}
      <MobileMenu
          ...
          <header className="sticky top-0 z-40 flex min-h-14 shrink-0 items-center border-b border-[var(--border)] bg-zinc-50/95 pt-[max(env(safe-area-inset-top,0px),0.4rem)] pr-[env(safe-area-inset-right,0px)] pl-[env(safe-area-inset-left,0px)] backdrop-blur-md dark:border-zinc-700/80 dark:bg-zinc-800/85">
            <div className="mx-auto flex h-full w-full max-w-7xl items-center gap-3 px-4">
                      ...
                      return (
                        <MotionLink
                          ...
                          className={[
                            ...
                            active ? navActiveClassesByHref(item.href) : navHeaderInactiveHoverByHref(item.href),
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] text-zinc-700 transition duration-200 hover:bg-zinc-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/60 focus-visible:ring-offset-2 dark:text-zinc-200 dark:hover:bg-zinc-800 dark:focus-visible:ring-offset-zinc-950"
